...
ReflectionShaderClass::ReflectionShaderClass(const ReflectionShaderClass& src) {}
...
bool ReflectionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"vsReflection.hlsli", L"psReflection.hlsli");
	if (!result)
		return false;
...
bool ReflectionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFileW(vsFilename, NULL, NULL, "ReflectionVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	result = D3DX11CompileFromFileW(psFilename, NULL, NULL, "ReflectionPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
											...
											D3DXMATRIX reflectionMatrix)
{
	...
	// Transpose ALL matrices (including the new reflection matrix) , as required by DirectX 11.
	...
	// Lock the reflection buffer so it can be written to.
	result = deviceContext->Map(m_reflectionBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
		return false;

	// Get a pointer to the reflection buffer.
	dataPtr2 = (ReflectionBufferType*)mappedResource.pData;

	// Copy the updated data to the buffer.
	dataPtr2->reflectionMatrix = reflectionMatrix;

	// Unlock the reflection buffer.
	deviceContext->Unmap(m_reflectionBuffer, 0);

	// Set the position of the reflection buffer in the vertex shader.
	bufferNumber = 1;

	// Set the reflection buffer in the vertex shader.
	...
	// Set the reflection texture in the pixel shader.
