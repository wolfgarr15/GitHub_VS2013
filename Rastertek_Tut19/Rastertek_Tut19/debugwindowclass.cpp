...
DebugWindowClass::DebugWindowClass(const DebugWindowClass& src) {}
...
bool DebugWindowClass::Initialize(ID3D11Device* device, int screenWidth, int screenHeight, int bitmapWidth, int bitmapHeight)
{
	...
	// Maintain the screen aspect ratio in the debug window.
	W_ratio = (float)m_screenWidth / (float)m_bitmapWidth;
	H_ratio = (float)m_screenHeight / (float)m_bitmapHeight;

	if (H_ratio > W_ratio)
		m_bitmapWidth = (int)(m_screenWidth / H_ratio);
	else
		m_bitmapHeight = (int)(m_screenHeight / W_ratio);
...
bool DebugWindowClass::Render(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	// Update the dynamic vertex and index buffers.
	result = UpdateBuffers(deviceContext, positionX, positionY);
	if (!result)
		return false;

	// Put the updated buffers on the render pipeline.
