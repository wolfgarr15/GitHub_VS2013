...
TransparentShaderClass::TransparentShaderClass(const TransparentShaderClass& src) {}
...
bool TransparentShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"vsTransparent.hlsli", L"psTransparent.hlsli");
	if (!result)
		return false;
...
bool TransparentShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFileW(vsFilename, NULL, NULL, "TransparentVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	result = D3DX11CompileFromFileW(psFilename, NULL, NULL, "TransparentPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	// Setup the description of the transparency buffer.
...
void TransparentShaderClass::ShutdownShader()
{
	...
	// Release the transparent buffer.
											...
											ID3D11ShaderResourceView* texture, float blendAmount)
{
	...
	// Lock the transparent buffer, so it can be written to.
	...
	// Copy the data to the transparent buffer.
	dataPtr2->blendAmount = blendAmount;

	// Unlock the transparent buffer.
	deviceContext->Unmap(m_transparentBuffer, 0);

	// Set the position of the transparent buffer in the pixel shader.
	bufferNumber = 0;

	// Set the transparent buffer in the pixel shader.
